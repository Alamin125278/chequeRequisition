    ...
    <div class="bg-white rounded-lg shadow-lg p-6 border border-gray-100">
      ...
      >
        <h1 class="text-2xl font-bold text-gray-800 mb-4 md:mb-0">
          <span class="text-emerald-600">Dispatch</span> Requisitions
        </h1>
      </div>
      ...
      <div class="mb-8 p-4 bg-gray-50 rounded-lg border border-gray-100">
        <div class="flex flex-wrap gap-4">
          ...
          >
            ...
            <a-select-option value="Main">Main Bank</a-select-option>
            <a-select-option value="North">North Bank</a-select-option>
            <a-select-option value="South">South Bank</a-select-option>
            <a-select-option value="East">East Bank</a-select-option>
            <a-select-option value="West">West Bank</a-select-option>
          </a-select>
        ...
        <div class="p-6 bg-gray-50 border-b border-gray-200">
          <div class="flex flex-wrap justify-between items-center">
            ...
            <div class="flex gap-2 mt-4 md:mt-0">
              <a-button
                v-if="selectedRequisition.status === 'Dispatch'"
                type="primary"
                class="bg-green-600 hover:bg-green-700 border-0"
                @click="deliveredRequisition(selectedRequisition)"
              >
...
export default defineComponent({
  ...
  setup() {
    ...
    const columns = [
      {
        ...
        dataIndex: "id",
        key: "id",
        sorter: (a: Requisition, b: Requisition) => a.id - b.id,
      },
      {
    ...
    const filteredRequisitions = computed(() => {
      ...
      if (searchText.value) {
        ...
        result = result.filter(
          (req) =>
            ...
            req.challanNo.toLowerCase().includes(searchLower) ||
      ...
      // Apply status filter
      if (bankFilter.value) {
        result = result.filter((req) => req.status === bankFilter.value);
      }
      // Apply branch filter
      ...
      // Apply dispatch status filter - only show dispatch requisitions
      result = result.filter((req) => req.status === "Dispatch");
    ...
    const generateMockData = (): Requisition[] => {
      ...
      return Array.from({ length: 20 }, (_, i) => ({
        id: i + 1,
        bankName: `Bank ${Math.floor(Math.random() * 10) + 1}`,
        branchName: `Branch ${Math.floor(Math.random() * 10) + 1}`,
        challanNo: `CH-${2025}-${1000 + i}`,
    ...
    const deliveredRequisition = (record: Requisition) => {
      confirmModalTitle.value = "Delivered Requisition";
      confirmModalMessage.value = `Are you sure you want to deliver requisition ${record.requisitionNo}?`;
      confirmModalOkText.value = "Deliver";
      confirmModalAction.value = "delivered";
    ...
    const handleConfirmAction = () => {
      ...
      } else if (confirmModalAction.value === "Delivered") {
        // Update status to Delivered
        requisitions.value[recordIndex].status = "Delivered";
        message.success(
          `Requisition ${record.requisitionNo} has been delivered`
        );
      }
